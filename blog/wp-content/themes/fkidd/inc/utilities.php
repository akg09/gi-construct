...
function fkidd_show_social_sites( $before,
	...
	if ( array_key_exists( 'social_facebook', $options )
		...
		fkidd_show_single_social_site( $separatorBefore, $separatorAfter, "https://www.facebook.com/pages/Girish-Constructions/256269554568614?ref=hl",
				__( 'Follow us on Facebook', 'fkidd'), 'facebook'.$iconSize, $openInNewWindow );
	}
	
	if ( array_key_exists( 'social_googleplus', $options )
		 && $options['social_googleplus'] != '' ) {
		fkidd_show_single_social_site( $separatorBefore, $separatorAfter, "https://plus.google.com/+Girishconstructionsbuy-sell-rent/about",
	...
	if ( array_key_exists( 'social_youtube', $options )
		...
		fkidd_show_single_social_site( $separatorBefore, $separatorAfter, $options[ 'social_youtube' ],
				__( 'Follow us on Twitter', 'fkidd'), 'youtube'.$iconSize, $openInNewWindow );
	}
...
function fkidd_show_website_logo_image_or_title() {
	...
	if ( get_header_image() != '' ) {
		...
		echo '<a href="http://www.girishconstructions.com/" title="'.get_bloginfo('name').'">';
